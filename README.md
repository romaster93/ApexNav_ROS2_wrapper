...
### ⚡ `isaacsim-ffw-swerve` Branch — IsaacSim + Swerve Drive Integration

> **This branch contains modifications specific to NVIDIA IsaacSim 5.1.0 + ROBOTIS FFW-SG2 Swerve robot.**
> The original ApexNAV code is preserved on the [`main` branch](https://github.com/romaster93/ApexNav_ROS2_wrapper).
>
> 📖 **Full Guide**: [isaacsim-aiworker-guide Step 10](https://github.com/romaster93/isaacsim-aiworker-guide/blob/main/guides/10-apexnav-autonomous.md)

#### Key Changes

| File | Description |
|------|-------------|
| `kino_astar.h/cpp` | Added `is_swerve` flag — axis-aligned collision check (replaces Ackermann rotated box) |
| `map_ros.cpp/h` | Separated `free_ray_cloud2d_` for max-range depth — prevents phantom walls in open environments |
| `sdf_map2d.cpp` | `checkCollision` OPTIMISTIC mode — allows traversal through UNKNOWN cells |
| `exploration_fsm_traj.cpp` | Relaxed odom_far threshold 0.6→1.5m (accommodates swerve P-control characteristics) |
| `exploration_manager.h` | `searchFrontierPath` Astar2D OPTIMISTIC + FREE cell walkback |
| `algorithm_traj.launch.py` | IsaacSim parameters (fx/fy=245.33, frame_id=World, max_depth from shared YAML) |
| `planning_param_ffw.yaml` | FFW-SG2 measured dimensions (length=0.56, width=0.51, is_swerve=true) |

#### Why Modify KinoAstar?

ApexNAV's KinoAstar was designed for **Ackermann (car-like) robots**:
- Movement direction = robot heading → yaw-rotated bounding box for collision checks
- **Swerve robots** can strafe laterally → yaw ≠ movement direction → rotated box causes false collisions
- Result: `isCollisionPosYaw occ!!!!!!!!` flooding → trajectory failure → premature FINISH

Setting `is_swerve: true` switches to an axis-aligned bounding box, resolving the issue. The original Ackermann logic is preserved in the `else` branch.

#### Running with IsaacSim
...
# Launch (with IsaacSim in Play state)
ros2 launch exploration_manager exploration_traj.launch.py
```

> For the full execution sequence (bridge, swerve_controller, etc.), see
> [Step 10 Guide](https://github.com/romaster93/isaacsim-aiworker-guide/blob/main/guides/10-apexnav-autonomous.md).
